...
UI/registration order (series RndTrig convention + third-pass reshuffle):
triggers with RndTrig immediately after them (drain, teleport, pipes,
sparkle, rndTrig), then Speed, pattern params, rotation with RstRot directly
after RotY, Audio, CapDiv (14 total — over the ~12 series guideline; nothing
was obviously droppable given the feature set).
...
| `speed` | 0.5..4 (ordinal subrange) | candidate | CURATE: musical mid-band; excludes PAUSE (RndTrig must not silently freeze the pattern) and the blazing top of the ladder |
