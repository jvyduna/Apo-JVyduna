...
public class Pipes3D extends ApotheneumPattern {
  ...
  /** Ring of recent bar rows, so new bars land on fresh Y heights */
  private final int[] recentBarY = new int[BAR_Y_HISTORY];
  private int recentBarYCount = 0, recentBarYNext = 0;

  /** When >= 0, walkStep() takes this direction instead of choosing (HoldBars) */
  ...
  public Pipes3D(LX lx) {
    ...
    addParameter("holdBars", this.holdBars);
  ...
  private boolean walkStep(int i) {
    ...
    // HoldBars overrides the choice entirely via forceDir (bars are straight
    // +x fills, then ±z depth parks), intersecting when the row is occupied.
    final int straight = this.pDir[i];
    int dir = -1;
    boolean intersecting = false;
    if (this.forceDir >= 0) {
      dir = this.forceDir;
      intersecting = (this.freeRun[dir] == 0);
      if (this.boundRun[dir] == 0) {
        dir = -1; // no in-bounds cell that way (defensive; callers pre-check)
      }
    } else if ((straight >= 0) && (this.freeRun[straight] >= 1) && (this.random.nextDouble() < P_STRAIGHT)) {
      dir = straight;
    } else {
      int total = 0;
      for (int d = 0; d < 6; ++d) {
  ...
  private boolean barWalkStep(int i, int dir) {
    this.forceDir = dir;
    final boolean ok = walkStep(i);
    this.forceDir = -1;
    return ok;
  }
  ...
  private boolean isRecentBarY(int y) {
    for (int k = 0; k < this.recentBarYCount; ++k) {
      if (this.recentBarY[k] == y) {
        return true;
      }
    }
    return false;
  ...
  private void recordBarY(int y) {
    this.recentBarY[this.recentBarYNext] = y;
    this.recentBarYNext = (this.recentBarYNext + 1) % BAR_Y_HISTORY;
    if (this.recentBarYCount < BAR_Y_HISTORY) {
      ++this.recentBarYCount;
    }
  }
  ...
  private void placePipe(int i, int x, int y, int z) {
    ...
    this.pDir[i] = -1;
    this.pSeg[i] = -1;
    // HoldBars: every placement starts a fresh FILLING bar on this row
   ...
   *  In HoldBars mode the landing is constrained to a fresh bar start
   *  (interior-left half, un-recent Y) — the manual Teleport trigger and the
   *  parked auto-advance both route through here. */
